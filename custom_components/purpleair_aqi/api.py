...
from .const import PM25_AVERAGING_FIELD_MAP
...
class PurpleAirConfig:
    ...
    averaging_period: str  # "real-time", "10min", "30min", "60min"
...
class PurpleAirClient:
    ...
    def _determine_pm25_field(self) -> str:
        # cf_1 conversions require real-time CF=1 data — averaging period is ignored for these
        conv = (self._config.conversion or "").lower()
        if conv in ("lrapa", "woodsmoke", "cf=1"):
            return "pm2.5_cf_1"
        return PM25_AVERAGING_FIELD_MAP.get(self._config.averaging_period, "pm2.5")
    ...
    def _process_response(
        self, payload: Dict[str, Any], pm25_field: str
    ) -> PurpleAirResult:
        ...
        for row in rows:
            ...
            pm25_raw = (
                float(row[field_index[pm25_field]])
                if row[field_index[pm25_field]] is not None
                else None
            )
            humidity = None
            ...
            if (
                ...
                and "latitude" in field_index
                and "longitude" in field_index
            ):
                lat = float(row[field_index["latitude"]])
                lon = float(row[field_index["longitude"]])
                coords: tuple[float, float] = (lat, lon)
            else:
                coords = base_coords or (0.0, 0.0)

            position_rating = -1
            if (
                "position_rating" in field_index
                and row[field_index["position_rating"]] is not None
            ):
        ...
        if (
            self._config.weighted
            and self._config.device_search
            and base_coords is not None
        ):
...
def aqi_linear(
    aqi_high: int, aqi_low: int, conc_high: float, conc_low: float, conc: float
) -> int:
...
def lrapa_conversion(pm: float) -> float:
    # LRAPA formula is only validated for PM2.5 < 65 µg/m³; return raw value above that range.
    if pm >= 65:
        return pm
    c = 0.5 * pm - 0.66
    return max(c, 0.0)
...
def distance2degrees(latitude: float) -> Tuple[float, float]:
    lat_miles_per_degree = 69.172
    lon_miles_per_degree = 69.172 * math.cos(math.radians(latitude))
